...
new Promise(resolve => resolve({ foo: "bar" })).then(result =>
  console.log(result)
);
// <- { foo: 'bar' }
...
Promise.all([Promise.reject(), fetch("/"), fetch("foo")])
  .then(responses => responses.map(response => response.statusText))
  .then(status => console.log(status.join(", ")));
// nothing happens
 ...
 */
Promise.race([fetch("/"), fetch("foo")]).then(response =>
